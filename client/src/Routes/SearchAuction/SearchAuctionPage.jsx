...
function SearchAuctionPage({ params }) {
  const [sortKey, setSortKey] = useState('startTime');
  const getSearchAuctionUrl = (word, size) => {
    return (page) => `/api/auction/search?word=${word}&page=${page}&size=${size}&sort=${sortKey}`;
  };
  const sortKeys = [
  ...
  return (
    <Container>
      ...
      <InfiniteAuctionList
        getUrl={getSearchAuctionUrl(params, 9)}
        queryKey={['auction' + params + sortKey]}
